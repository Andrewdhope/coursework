...
# Split on fips and take only Baltimore City data
    splitted <- split(nei, nei$fips)
    baltimore <- splitted[["24510"]]
    
# Plot the sum of PM2.5 emissions for each year using base plot
    yearTotals <- tapply(baltimore$Emissions, baltimore$year, sum)
    png(file = "plot2.png")
    plot(unique(baltimore$year), yearTotals, xlab = "Year", ylab = "Total PM2.5 Emissions")
    dev.off()
    return()
}