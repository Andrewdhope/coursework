...
# grep on EI.Sector contains Coal
    ...
    mergedCoal <- merged[coal,]
    
# Plot the sum of PM2.5 emissions for each year using base plot
    yearTotals <- tapply(mergedCoal$Emissions, mergedCoal$year, sum)
    png(file = "plot4.png")
    plot(unique(mergedCoal$year), yearTotals, xlab = "Year", ylab = "Total PM2.5 Emissions")
    dev.off()
    return()
}