...
# grep on EI.Sector containing Mobile
    mobile <- grep("Mobile", merged$EI.Sector)
    mergedMobile <- merged[mobile,]
    
# Plot the sum of PM2.5 emissions for each year using base plot
    yearTotals <- tapply(mergedMobile$Emissions, mergedMobile$year, sum)
    png(file = "plot5.png")
    plot(unique(mergedMobile$year), yearTotals, xlab = "Year", ylab = "Total PM2.5 Emissions")
    dev.off()
    return()
}